...
#Redis slow log文件
REDIS_SLOW_LOG = "/tmp/redis_slow.log"
...
def get_data():
	if REDIS_PWD.strip():
		command( """ %s -h %s -p %s -a %s -c info 2>/dev/null > %s """ %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(), REDIS_PWD.strip(), REDIS_FILE.strip() ) )
	else:
		command( """ %s -h %s -p %s -c info 2>/dev/null > %s """ %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(), REDIS_FILE.strip() ) )
...
def redis_discovery():
	t=os.popen(""" ps -ef|grep "redis"|grep -w "\[cluster\]"|awk '{print $9}'|awk -F ":" '{print $2}' """)
	ports = []
	for port in  t.readlines():
		r = os.path.basename(port.strip())
		ports += [{'{#REDIS_NODE_PORT}':r}]
	return json.dumps({'data':ports},sort_keys=True,indent=4,separators=(',',':'))

#监控redis是否挂起
def redis_run_status():
	#ping
	cmd_a = "%s -h %s -p %s -a -c %s ping" %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(), REDIS_PWD.strip())
	cmd_b = "%s -h %s -p %s -c ping" %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip())
	if REDIS_PWD.strip():
...
def redis_slowlog():
	try:
		...
		num = 0	
		if REDIS_PWD.strip():
			res_a = command(""" %s -h %s -p %s -a %s -c CONFIG GET slowlog-max-len|grep -v "slowlog-max-len" """ %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(), REDIS_PWD.strip()))
			res_b = command(""" %s -h %s -p %s -a %s -c slowlog get %s |grep -aE "^[1-9]" > %s """ %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(), REDIS_PWD.strip(), res_a[1].strip(), REDIS_SLOW_LOG))
		else:
			res_a = command(""" %s -h %s -p %s -c CONFIG GET slowlog-max-len|grep -v "slowlog-max-len" """ %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip()))
			res_b = command(""" %s -h %s -p %s -c slowlog get %s |grep -aE "^[1-9]" > %s""" %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(),res_a[1].strip(),REDIS_SLOW_LOG)) 
...
def redis_info():
	try:
		...
		#监控redis能否正常读取
			...
			#获取随机key
			cmd_a = "%s -h %s -p %s -a -c %s RANDOMKEY" %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip(), REDIS_PWD.strip())
			cmd_b = "%s -h %s -p %s -c RANDOMKEY" %(REDIS_BIN.strip(),REDIS_HOST.strip(), REDIS_PORT.strip())
			if REDIS_PWD.strip():
				res = command(cmd_a)
				if res[0] == 0:
					return 1
